    ...
    return safeCall(async () => {
      ...
      const r = await fetch('https://api.together.xyz/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          // v36-R5：Kimi-K2.6 是 reasoning model，chain-of-thought 跟最終答案共用同一個 max_tokens 額度。
          // 原本 3000 對這個案子的複雜度（5張圖 + 5段式格式要求）不夠，思考還沒寫完就被截斷，
          // 導致 content 永遠是空的，程式碼只能 fallback 去讀還在思考中、也被截斷的 reasoning 欄位。
          // 拉高到 8000，讓他有機會先想完，再把乾淨的最終答案寫進 content。
          max_tokens: 8000,
      ...
      // v36-R5：如果 content 還是空的（極端複雜案件仍可能發生），fallback 用 reasoning 時
      // 加上明確標記，讓前端 parseStance() 和 Maxie 都能一眼看出這不是正式定稿的答案，
      // 而是被截斷的思考過程，避免誤以為是 Kimi 正式的判決文字。
      if (msg2?.content) return msg2.content;

      if (msg2?.reasoning) return `⚠️【思考未完成，以下為截斷的推理過程，非正式定稿】\n\n${msg2.reasoning}`;
