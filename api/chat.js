...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ===== 🛋️ 第1層 B：Grok 4.20（支援圖片）=====
    if (apiId === 'grok') {
      if (!GROK_API_KEY) return res.status(200).json({ reply: '🛋️ Grok 的API Key還沒設定～' });
      try {
        const body = {
          model: 'grok-4.20-reasoning',
          input: buildPersonality('grok') + '
...
' + message
        ...
        const d = await r.json();
        if (d.error) return res.status(200).json({ reply: `⚠️ Grok錯誤：${d.error.message || JSON.stringify(d.error)}` });
        const replyText = d.output?.[0]?.content?.[0]?.text || d.output_text || '🛋️ Grok 在沉思中～';
        return res.status(200).json({ reply: replyText });
      } catch(e) {
        return res.status(200).json({ reply: `⚠️ Grok連線失敗：${e.message}` });
      }
    }
