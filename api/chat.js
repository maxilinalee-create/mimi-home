...
5. 直接從具體內容開始，不要廢話`);
  ...
  // ===== v27：callLlama（駱馬偵探，Llama 4 Maverick via Groq）=====
  async function callLlama(msg) {
    ...
    try {
      // v27修正：改走 Groq（免費、穩定、支援 Llama 4 Maverick）
      const r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'meta-llama/llama-4-maverick-17b-128e-instruct', // Groq 上的 Llama 4 Maverick
          messages: [
            { role: 'system', content: systemPrompt },
