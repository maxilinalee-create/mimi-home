...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ===== ✨ Gemini（Grok 2026穩定修正版：支援圖片）=====
    if (apiId === 'gemini') {
      if (!GEMINI_API_KEY) return res.status(200).json({ reply: '✨ Gemini 的 API Key 還沒設定～' });

      const parts = [];
      // 文字部分（人格 + prompt 合在一起）
      const fullPrompt = `${buildPersonality(apiId)}\n\n${message}`;
      ...
      if (d.error) {
        ...
        return res.status(200).json({
          reply: `⚠️ Gemini 暫時無法處理：${d.error.message || '未知錯誤'}`
        });
      }
      const replyText = d.candidates?.[0]?.content?.parts?.[0]?.text || '✨ Gemini 正在沉思中...';
